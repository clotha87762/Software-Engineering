...
/********************************************************/
token_stream open_token_stream(fname, tok)
char *fname;
token tok
{
 token_stream fp;
...
{ 
  ...
  int id=0;
  char ch,ch1[2], ch2[2];
  for (j=0;j<=81;j++)          /* initial the buffer   */
      { buffer[j]='\0';} 
   ch1[0]='\0';
   ch1[2]='\0';
   ch=get_char(tp);
   ...
   if(is_eof_token(ch2)==TRUE)       /* if end character is eof token    */
      { ch=unget_char(ch,tp);        /* then put back eof on token_stream */
        if(ch==EOF)unget_error(tp);
        return(buffer);
      }
   if(is_spec_symbol(ch2)==TRUE)     /* if end character is special_symbol */
...
{ char ch1[2];  /* fixed array declaration MONI */
 ...
 if(str_com_id==1 && FALSE)          /* is string token */
...
{ 
 ...
 if(is_end_of_character_stream(tok))return(char_stream)
...
{ 
 if (!strcmp(str,"and") || !strcmp(str,"or") || !strcmp(str,"if") ||
    !strcmp(str,"xor")||!strcmp(str,"lambda")||!strcmp(str,"=>"))
      return(TRUE);
 else if (!strcmp(str,"or") ||!strcmp(str,"xor")||!strcmp(str,"lambda"))
      return(TRUE);
