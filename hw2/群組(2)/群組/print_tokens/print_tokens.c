...
static token numeric_case(token t,token tt ,char c,char cc,int i);
static token error_or_eof_case(token t,token tt,int i,int ii,char c);
static int check_delimiter(char c);
...
static get_actual_token(char token_str[]);
...
character_stream stream_ptr;
{
      if(stream_ptr->stream[stream_ptr->stream_ind] == EOF )
            return(TRUE);
      else
            return(FALSE);
}
...
token_stream tstream_ptr;
{
      ...
      while(!token_found)
      {
	  if(token_ind < 81) /* ADDED ERROR CHECK - hf */
	  {
...
int state;
{
      ...
      {   /* Return the respective macro for the Keyword. */
          case 6 : return(LAMBDA);//fprintf(stdout, "pass\n");break;
          case 9 : return(AND);//fprintf(stdout, "pass\n");break;
          case 11: return(OR);//fprintf(stdout, "pass\n");break;
          case 13: return(IF);//fprintf(stdout, "pass\n");break;
          case 16: return(XOR);//fprintf(stdout, "pass\n");break;
...
char token_str[];
{
     ...
     {   /* Return the respective CONSTANT macro. */
         ...
         case 31 : return(CHARACTER_CONSTANT);
         default : break;
     }
}
...
/* *********************************************************************
   ...
   Output        : void
   Exceptions    : none.
   Description   : This function clears all the tokens records.
 * ***************************************************************** */

static reset_token(token_str,token_ind)
int token_ind;
char token_str[];
{
          int ind,start;
