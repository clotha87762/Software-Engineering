...
char	*pat;
{
    ...
    junk = addstr(ENDSTR, pat, &j, MAXPAT, 0);
    if ((done) || (arg[i] != delim))
	result = 0;
    else
	if ((!done))
...
int
getpat(arg, pat, result)
char*	arg;
char*	pat;
{
    int	makeres;

    makeres = makepat(arg, 0, ENDSTR, pat, makeres);
    return (makeres > 0);
}
...
char	*argv[];
{
   ...
   if (!result)
   {
       (void)fprintf(stdou, "change: illegal \"from\" pattern\n");
       exit(2);
   }
   ...
   if (argc >= 3)
   {
       ...
       if (!result)
       {	
	   (void)fprintf(stdout, "change: illegal \"to\" string %s\n");
	   exit(3);
       }
   } else
   {
